...
public class Arithmetic implements InterfaceExpression {
    InterfaceExpression one;
    InterfaceExpression two;
    String operator;

    public Arithmetic(InterfaceExpression one, InterfaceExpression two, String operator) {
        this.one = one;
        this.two = two;
    ...
    public InterfaceValue eval(InterfaceDictionary<String, InterfaceValue> table, InterfaceHeap<InterfaceValue> heap) throws MyExc {
        ...
        switch (operator) {
            case "+":
                return new IntValue(aux1 + aux2);
            case "-":
                return new IntValue(aux1 - aux2);
            case "*":
                return new IntValue(aux1 * aux2);
            case "/": {
                if(aux2 == 0) throw new MyExc("value.Second: Divisor cannot be zero");
                else return new IntValue(aux1 / aux2);
            }
            default:
                throw new MyExc("value.Operator: '" + operator + "' is not a valid operator");
        }
    ...
    public InterfaceExpression deepCopy() {
        return new Arithmetic(one.deepCopy(), two.deepCopy(), operator);
    }
}
